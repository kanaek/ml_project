...
def get_holidays(fpath):
    ...
    hh2 =  pd.DataFrame({"date2":lines})
    hh2.to_csv('model/hh2.csv')
    return hh2
...
def get_holiday_names(fpath):
    ...
    hh =  pd.DataFrame({"date2":lines_dt, "holiday_name":lines_hol})
    hh.to_csv('model/hh.csv')
    return hh
...
def preprocess(_df, is_train):
    ...
    wtr['preciptotal_flag'] = wtr['preciptotal'].map(lambda x: get_precip(x))
...
'''
# preprocess 
df_train = pd.read_csv("model/testtest.csv")
...
preprocess(df_train, True).to_csv('model/train2.csv')
...
preprocess(df_test,  False).to_pickle('model/test2.csv')
'''